...
def olr_wa_sccm_call(X, y, w_base, w_inc, base_model_size, increment_size, kpi, multiplier, n_samples,
                     sccm_window_size=4, used_kpi_window_size=4,
                     safe_band=0.005, enable_recalibration=True,
                     max_recalibration_batches=5):
    ...
    for iteration, mini_batch_uuid, Xj, yj in mini_batch_generator:
        ...
        if (len(memoryManager.mini_batch_data) >= sccm_window_size):
            ...
            )  # contains the configurable short-term window
            ...
            # The safe band suppresses reactions to very small KPI deviations.
            ST_drift_detected = bool(ST_drift_detected and drift_magnitude > float(safe_band))
            ...
            if (ST_drift_detected):
                ...
                # For OLR-WA, larger w_inc gives more weight to the
                # incremental model and is therefore more adaptive.
                # After drift detection, do not allow SCCM to reduce
                # w_inc below the base/default value for this mini-batch.
                tuned_w_inc = max(raw_tuned_w_inc, float(w_inc))
                tuned_w_inc = float(np.clip(tuned_w_inc, 0.5, 0.995))

                if tuned_w_inc != raw_tuned_w_inc:
                    print(f"raw_tuned_w_inc {raw_tuned_w_inc} adjusted_to {tuned_w_inc}")
                ...
                if LT_drift_detected and enable_recalibration:
                    ...
                    counter = 0
                    max_no_of_mini_batches_requests = int(max_recalibration_batches)
    ...
    return base_coeff, accuracy_list, mse_list
