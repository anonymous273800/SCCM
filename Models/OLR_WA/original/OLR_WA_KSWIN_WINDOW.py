...
def olr_wa_regression_kswin_window(
    ...
    - Feed KSWIN with PER-SAMPLE squared error using the OLR-WA candidate model
    ...
    for i in range(no_of_base_model_points, n_samples - no_of_base_model_points, increment_size):
        ...
        if drift_detected_in_this_batch:
            ...
            # discard pre-drift window and start new post-drift window
            ...
            # no new drift: keep growing / sliding the recent post-drift window
            recent_batches_X.append(Xj.copy())
            recent_batches_y.append(yj.copy())

        # --------------------------------------------------------
        # TRUE WINDOW RETRAIN:
        # rebuild using the entire current recent window
        # --------------------------------------------------------
