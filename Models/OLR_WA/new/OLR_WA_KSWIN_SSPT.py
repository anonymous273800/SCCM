...
def olr_wa_regression_adversarial_kswin_sspt(
    X_train,
    y_train,
    w_base,
    w_inc,
    ...
    final_coeff, acc_list, mse_list = olr_wa_regression_kswin_sspt(
        X=X_train,
        y=y_train,
        w_base=w_base,
        w_inc=w_inc,
...
def olr_wa_regression_kswin_sspt(
    X,
    y,
    w_base,
    w_inc,
    ...
    """
    OLR-WA with KSWIN-triggered SSPT hyperparameter tuning.

    Policy:
    - Build the normal OLR-WA candidate on each mini-batch.
    - Feed KSWIN with per-sample squared error.
    - If KSWIN detects drift inside the batch, run SSPT on the CURRENT batch
    ...
    # ------------------------------------------------------------
    # 2) Online OLR-WA with KSWIN-based SSPT tuning
    # ------------------------------------------------------------
    for i in range(no_of_base_model_points, n_samples - no_of_base_model_points, increment_size):
        ...
        # --------------------------------------------------------
        # Feed KSWIN with PER-SAMPLE squared error using candidate
        # --------------------------------------------------------
        ...
        if drift_detected_in_this_batch:
            print(f"KSWIN drift detected at mini-batch {mini_batch_counter}. SSPT tuning activated.")
            ...
            if tuned_coeff is not None:
                ...
                y_pred_tuned = Predictions._compute_predictions__(Xj, tuned_coeff)
                tuned_mse = np.mean(np.square(yj - y_pred_tuned))

                if tuned_mse <= candidate_mse:
                    print("SSPT ACCEPTED")
                    ...
                    print(
                        ...
                        f"score={tuned_score:.5f}, tuned_MSE={tuned_mse:.5f}, "
                        f"candidate_MSE={candidate_mse:.5f}"
                    )
                else:
                    print("SSPT REJECTED; using normal OLR-WA candidate")
                    base_coeff = candidate_coeff
            else:
                print("SSPT failed to find a candidate; fallback to normal OLR-WA candidate.")
                base_coeff = candidate_coeff
        else:
            base_coeff = candidate_coeff
    ...
    print(f"Total KSWIN+SSPT tunings: {tuned_count}")
