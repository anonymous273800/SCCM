...
def olr_wa_regression_adwin_reset(
    ...
    for i in range(no_of_base_model_points, n_samples - no_of_base_model_points, increment_size):
        ...
        if drift_detected_in_this_batch:
            ...
            reset_coeff = fit_linear_model_as_plane(Xj, yj)
            ...
            y_pred_reset = Predictions._compute_predictions__(Xj, reset_coeff)
            reset_mse = np.mean(np.square(yj - y_pred_reset))

            if reset_mse <= candidate_mse:
                print("RESET ACCEPTED")
                base_coeff = reset_coeff
                w_base = default_w_base
                w_inc = default_w_inc
                reset_count += 1
            else:
                print("RESET REJECTED; using normal OLR-WA candidate")
