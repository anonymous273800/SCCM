...
def olr_wa_regression_adversarial_adwin_ohl(
    X_train,
    y_train,
    w_base,
    w_inc,
    ...
    final_coeff, acc_list, mse_list = olr_wa_regression_adwin_ohl(
        X=X_train,
        y=y_train,
        w_base=w_base,
        w_inc=w_inc,
...
def olr_wa_regression_adwin_ohl(
    X,
    y,
    w_base,
    w_inc,
    ...
    """
    OLR-WA with ADWIN-triggered lightweight OHL adaptation.

    Policy:
    - Build the normal OLR-WA candidate on each mini-batch.
    - Feed ADWIN with per-sample squared error.
    - If ADWIN detects drift inside the batch, update w_inc using a single
    ...
    # ------------------------------------------------------------
    # 2) Online OLR-WA with ADWIN-based OHL tuning
    # ------------------------------------------------------------
    for i in range(no_of_base_model_points, n_samples - no_of_base_model_points, increment_size):
        ...
        if drift_detected_in_this_batch:
            print(f"ADWIN drift detected at mini-batch {mini_batch_counter}. OHL update activated.")
            print("OHL ACTIVATED")
            ...
            if tuned_mse <= candidate_mse:
                ...
                print("OHL REJECTED; using normal OLR-WA candidate")
    ...
    print(f"Total ADWIN+OHL updates: {tuned_count}")
