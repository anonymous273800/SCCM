...
def select_tuned_lambda_from_scale(conceptDriftDetector, scale, drift_magnitude, DS=None, lambda_bounds=(0.90, 0.999)):
    try:
    ...
    tuned_lambda = conceptDriftDetector.get_value_for_range(
        drift_magnitude,
        map_ranges_values
    )

    tuned_lambda = float(np.clip(float(tuned_lambda), lambda_bounds[0], lambda_bounds[1]))
    return tuned_lambda
