    ...
    for iteration, mini_batch_uuid, Xj, yj in mini_batch_generator:
        ...
        if len(memoryManager.mini_batch_data) >= sccm_window_size:
            ...
            if ST_drift_detected:
                ...
                # For Widrow-Hoff, larger learning rate means faster adaptation.
                # After drift detection, do not allow SCCM to make the model
                # less adaptive than the base Widrow-Hoff learning rate.
                tuned_learning_rate = max(
                    float(tuned_learning_rate),
                    float(learning_rate)
                )
