...
def ad_widrow_hoff_generic(
    ...
    """
    SCCM wrapper used by the experiment file.
...
def _clip(value, bounds):
    return float(np.clip(float(value), bounds[0], bounds[1]))


def get_next_mini_batch(X, y, no_of_base_model_points, increment_size):
    n_samples_trn, _ = X.shape
    j = 0
...
def train(Xj, yj, w, learning_rate):
    """
    Widrow-Hoff mini-batch training with intercept support.
    """
    Xj_aug = add_intercept_to_X(Xj)
    yj = np.asarray(yj, dtype=float).reshape(-1)
...
def get_widrow_hoff_scale_map(conceptDriftDetector, scale, DS):
    return conceptDriftDetector.get_scales_map_widrow_hoff(scale, DS)
...
def widrow_hoff_sccm_call(
    ...
    learning_rate = _clip(learning_rate, lr_bounds)
    ...
    # Must remain 8 because get_scales_map_widrow_hoff uses result[8].
    num_intervals = 8
    ...
    for iteration, mini_batch_uuid, Xj, yj in mini_batch_generator:
        ...
        if len(memoryManager.mini_batch_data) >= 4:
            ...
            if ST_drift_detected:
                ...
                tuned_learning_rate = max(float(tuned_learning_rate), float(learning_rate))

                tuned_learning_rate = float(
                    np.clip(tuned_learning_rate, 1e-4, 0.05)
                )

                if tuned_learning_rate is None:
                    tuned_learning_rate = learning_rate

                tuned_learning_rate = _clip(tuned_learning_rate, lr_bounds)
                print("tuned_learning_rate", tuned_learning_rate)

                # Short-term retraining with tuned learning rate.
                ...
                # Compare original vs tuned on the same mini-batch.
                pred_orig = wh_predict_many(Xj, w_candidate)
                pred_tuned = wh_predict_many(Xj, w_candidate_tuned)

                mse_orig = safe_mse(yj, pred_orig)
                mse_tuned = safe_mse(yj, pred_tuned)
                ...
                if LT_drift_detected:
                    ...
                    while LT_drift_detected and counter < max_no_of_mini_batches_requests:
                        ...
                        try:
                            ...
                            # Correctly use next_Xj and next_yj.
                            w_next = train(
                                Xj=next_Xj,
                                yj=next_yj,
                                w=w_candidate.copy(),
