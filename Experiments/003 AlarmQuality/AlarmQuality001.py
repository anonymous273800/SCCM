...
Use this file when the original SCCM model files do NOT support
`return_event_log=True`. This script does not modify any existing project file.
It runs the original SCCM models, captures/parses their console output, and
builds the reviewer-requested alarm-quality tables.

Place this file at:
    Experiments/003 AlarmQuality/one_file_sccm_alarm_quality_NO_FLAG.py

Run smoke test, PowerShell:
    $env:SCCM_DATASETS="ADS01"
    $env:SCCM_SEEDS="42"
    python "Experiments/003 AlarmQuality/one_file_sccm_alarm_quality_NO_FLAG.py"

Run full experiment, PowerShell:
    Remove-Item Env:SCCM_DATASETS -ErrorAction SilentlyContinue
    Remove-Item Env:SCCM_SEEDS -ErrorAction SilentlyContinue
    python "Experiments/003 AlarmQuality/one_file_sccm_alarm_quality_NO_FLAG.py"
"""
...
def get_selected_datasets():
    selected_names = parse_env_list("SCCM_DATASETS", [])
    if not selected_names:
        return DATASETS
    selected_names = set(name.upper() for name in selected_names)
    return [item for item in DATASETS if item[0].upper() in selected_names]


def get_selected_seeds():
    raw = parse_env_list("SCCM_SEEDS", [])
    if not raw:
...
def run_all():
    ...
    candidate_source = os.environ.get("SCCM_CANDIDATE_SOURCE", "short_term").strip()
    cooldown_factor = float(os.environ.get("SCCM_COOLDOWN_FACTOR", "1.0"))
    ...
    with open(CONSOLE_LOG, "w", encoding="utf-8") as log_file:
        ...
        log_print("SCCM one-file alarm-quality experiment for ORIGINAL code")
