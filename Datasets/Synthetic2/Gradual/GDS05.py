...
def get_GDS05(
    ...
    if beta2 is None:
        beta2 = np.array([-3.0, -1.5, 2.0, -1.5, -2.5, 3.0, -2.0, 1.5, -2.0, 2.0], dtype=float)
...
def print_GDS05_info(X, y, X1, y1, X2, y2, meta):
    print_gradual_dataset_info(X, y, X1, y1, X2, y2, meta)

if __name__ == "__main__":
    X, y, X1, y1, X2, y2, meta = get_GDS05(return_meta=True)
    print_GDS05_info(X, y, X1, y1, X2, y2, meta)
    plot_gradual_dataset(
        X1, y1, X2, y2,
        title="GDS05: Gradual Drift in High-D Regression (High, Medium)"
    )
