...
def _build_segment_lengths(n_samples_total, n_segments=6):
    if n_samples_total < n_segments:
        raise ValueError(
            f"n_samples_total must be at least {n_segments}."
        )

    base_segment_size, remainder = divmod(
        n_samples_total,
        n_segments
    )

    return [
        base_segment_size + (1 if i < remainder else 0)
...
def generate_gradual_dataset(
    *,
    ...
    if n_features == 1:
        ...
        if beta1 is None or beta2 is None:
            raise ValueError(
                ...
                "beta1 and beta2 must be provided."
            )
        ...
        if (
            len(beta1_arr) != n_features
            or len(beta2_arr) != n_features
        ):
            raise ValueError(
                "Length of beta1 and beta2 must match n_features."
            )
    ...
    segment_lengths = _build_segment_lengths(
        n_samples_total=n_samples_total,
        n_segments=len(segment_concepts)
    )

    X_segments = []
    y_segments = []

    for seg_len, seg_concept in zip(
        segment_lengths,
        segment_concepts
    ):
        if seg_concept == "C1":
    ...
    concept_1_total_samples = sum(
        length
        for length, concept in zip(
            segment_lengths,
            segment_concepts
        )
        if concept == "C1"
    ...
    if return_meta:
        meta = {
            ...
            "n_switches": len(segment_lengths) - 1,
            "concept_1_total_samples": concept_1_total_samples,
            "concept_2_total_samples": concept_2_total_samples,
            "concept_1_params": {
                "mu": mu1,
                "sigma": sigma1,
                "beta": (
            ...
            "y_js_divergence": drift_metrics[
                "y_js_divergence"
            ],
            "y_wasserstein_distance": drift_metrics[
                "y_wasserstein_distance"
            ],
            "x_any_ks_significant": drift_metrics[
                "x_any_ks_significant"
            ],
            "x_min_ks_pvalue": drift_metrics[
                "x_min_ks_pvalue"
            ],
            "x_avg_js_divergence": drift_metrics[
                "x_avg_js_divergence"
            ],
            "euclidean_distance_start_end": (
                euclidean_distance
            ),
            "euclidean_distance_consecutive": (
                euclidean_distance
            ),
            "empirical_distance_coef_only": drift_metrics[
                "method_1_empirical"
            ],
            "empirical_distance_full_params": drift_metrics[
                "method_2_empirical"
            ]
        }
...
def plot_gradual_dataset(
    X1,
    y1,
    X2,
    y2,
    title="Gradual Drift Dataset"
):
