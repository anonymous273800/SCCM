...
def get_IDS04(
    ...
    if beta_end is None:
        beta_end = np.array([2.8, 1.4, -0.9, 0.6, 1.9, -2.3, 1.1, -0.4, 0.9, -1.0])
...
def print_IDS04_info(X, y, X_steps, y_steps, meta):
    print_incremental_dataset_info(X, y, X_steps, y_steps, meta)


if __name__ == "__main__":
    X, y, X_steps, y_steps, meta = get_IDS04(return_meta=True)
    print_IDS04_info(X, y, X_steps, y_steps, meta)
    plot_incremental_dataset(
        X_steps,
        y_steps,
        title="IDS04: Incremental Drift in High-D Regression (High, Small)"
    )