...
DIRECTORY_NAME = "DriftDetectionQuality2"
RESULTS_DIRECTORY_NAME = "BaselineResults"
...
class ActivityRecord:
    ...
    def as_row(self) -> dict[str, Any]:
        ...
        return {
            ...
            "processing_increment": self.processing_increment,
            ...
            "intervention_events": self.adaptation_activations,
            "adaptations_per_1000": per_1000(self.adaptation_activations, self.processed_samples),
            "recalibrations_per_1000": 0.0,
            "interventions_per_1000": per_1000(self.adaptation_activations, self.processed_samples),
            "runtime_seconds": self.runtime_seconds,
            "runtime_per_1000_samples": per_1000(self.runtime_seconds, self.processed_samples),
...
class ActivityRegistry:
    def __init__(self, project_root: Path, model: str):
        ...
        self.current_seed = DEFAULT_SEEDS[0]
    ...
    def create_record(
        ...
        *,
        ...
        return ActivityRecord(
            ...
            processing_increment=metadata.processing_increment,
            processed_samples=metadata.monitored_samples,
        )
...
def discover_quality_configs(project_root_text: str) -> dict[tuple[str, str], tuple[dict[str, Any], Path]]:
    ...
    quality_root = project_root / "DriftDetectionQuality2"
...
def _find_dataset_module_alias(script_path: Path) -> str:
    ...
    for node in ast.walk(function_node):
        if not isinstance(node, ast.Call) or not isinstance(node.func, ast.Attribute):
            continue
        if node.func.attr.startswith("get_") and isinstance(node.func.value, ast.Name):
...
def build_experiment_metadata(
    ...
    quality_seeds = [int(value) for value in quality_config.get("seeds", DEFAULT_SEEDS)]
    ...
    if quality_seeds != DEFAULT_SEEDS:
        raise RuntimeError(
            f"Expected SCCM quality seeds {DEFAULT_SEEDS} for {model}/{dataset}, found {quality_seeds}"
        )

    getter = get_synthetic2_getter(dataset)
    quiet = io.StringIO()
    with contextlib.redirect_stdout(quiet), contextlib.redirect_stderr(quiet):
        output = getter(seed=DEFAULT_SEEDS[0], return_meta=True)
    ...
    return ExperimentMetadata(
        ...
        processing_increment=(
            int(quality_config.get("increment_user_value", 10))
            if model == "OLR-WA"
            else int(quality_config.get("report_interval", 1))
            if model == "WidrowHoff"
            else 1
        ),
    )
    ...
    *,
    ...
    def wrapped(*args: Any, **kwargs: Any):
        ...
        try:
            ...
            record.runtime_seconds = measurement.runtime_seconds
            record.rss_before_mb = measurement.rss_before_mb
            record.peak_rss_mb = measurement.peak_rss_mb
            record.peak_rss_delta_mb = measurement.peak_rss_delta_mb
            record.memory_measurement_method = measurement.measurement_method
...
def make_run_single_wrapper(
    ...
    def wrapped(*args: Any, **kwargs: Any):
        try:
            ...
            registry.current_seed = int(bound.arguments.get("seed", SEED))
        except (TypeError, ValueError):
            registry.current_seed = int(kwargs.get("seed", SEED))
...
def run_experiment_script(
    ...
    main_kwargs["seeds"] = list(DEFAULT_SEEDS)
...
def write_records(path: Path, records: list[ActivityRecord]) -> None:
    empty = ActivityRecord(
        drift_type="", dataset="", model="", seed=SEED, baseline="",
        ...
        processing_increment=1, processed_samples=0,
    )
...
def run_model(model: str) -> tuple[Path, Path]:
    ...
    for index, script_path in enumerate(scripts, start=1):
        ...
        records_path = raw_root / f"baseline_activity_{_file_safe_model(model)}.csv"
        failures_path = raw_root / f"baseline_failures_{_file_safe_model(model)}.csv"
        write_records(records_path, registry.records)
        write_failures(failures_path, registry.failures)

    expected_rows = len(scripts) * 8 * len(DEFAULT_SEEDS)
    error_records = sum(record.status != "ok" for record in registry.records)
    inconsistent_records = sum(
        record.reported_activation_total != ""
        and int(record.reported_activation_total) != record.adaptation_activations
    ...
    print(f"Method-level errors: {error_records}")
    print(f"Activation-count inconsistencies: {inconsistent_records}")
    print(f"Detailed results: {records_path}")
    print(f"Failure log: {failures_path}")
    if len(registry.records) != expected_rows or registry.failures or error_records:
        raise RuntimeError(
            f"Incomplete baseline run for {model}: rows={len(registry.records)}/{expected_rows}, "
            f"experiment_failures={len(registry.failures)}, method_errors={error_records}."
        )
...
def cli_main(default_model: str | None = None) -> None:
    parser = argparse.ArgumentParser(
        description=(
            ...
            "using the exact Synthetic2 data and SCCM model settings over the five predictive seeds."
        )
    )
    parser.add_argument(
        "--model",
