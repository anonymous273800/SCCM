...
OLR_WA_FOLDER = SUMMARY_FOLDER.parent

DATASETS = ["ADS01", "ADS02", "ADS03", "ADS04", "ADS05", "ADS06"]

OUTPUT_BY_DATASET = SUMMARY_FOLDER / "OLR_WA_Abrupt_by_dataset.csv"
OUTPUT_OVERALL = SUMMARY_FOLDER / "OLR_WA_Abrupt_overall.csv"
...
def find_dataset_summary(dataset):
    dataset_number = dataset.replace("ADS", "").zfill(3)

    experiment_folder = (
        OLR_WA_FOLDER
        / f"{dataset_number}-OLR-WA-{dataset}"
    )
...
def get_aggregate_row(summary_file):
    ...
    # For compatibility with older single-seed files.
    return rows[-1]
...
def calculate_overall_result(dataset_rows):
    ...
    for row in dataset_rows:
        ...
        mean_delay = to_float(row.get("mean_delay"))
...
def main():
    ...
    for dataset in DATASETS:
        ...
        dataset_rows.append({
            "dataset": dataset,
            "model": row.get("model", "OLR-WA-SCCM"),
    ...
    print("\n" + "=" * 40)
    print("OLR-WA ABRUPT OVERALL RESULT")
    print("=" * 40)
